...
import { X, Send, Bot, PanelRightClose, Trash2, Sparkles, User, BarChart2, Table as TableIcon, ExternalLink, Settings, Save, Key, Eye, EyeOff, AlertCircle, ChevronDown, Maximize, Minimize, Image as ImageIcon } from 'lucide-react';
...
const RenderImage: React.FC<{ src: string }> = ({ src }) => (
  <div className="my-4 rounded-xl border border-slate-200 shadow-sm overflow-hidden bg-white relative group">
    ...
    <div className="absolute top-2 right-2 flex items-center gap-1.5 px-2 py-1 bg-black/50 text-white/80 rounded-full text-[10px] font-bold backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity">
      <ImageIcon size={10}/>
